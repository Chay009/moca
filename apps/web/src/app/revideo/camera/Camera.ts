...
import { Rect, type RectProps, Curve } from '@revideo/2d';
import { signal, cloneable } from '@revideo/2d/lib/decorators';

/*
This whole component works but the only issue is when we clicked on ele which is inside camera it is not notified to inspector
so it's state is not being updated that can be done in scene2d compone which is internal
https://github.com/motion-canvas/motion-canvas/pull/1019/files
*/
...
export class Camera extends Node {
     ...
     */
    public scene = createSignal<Node>(new Node({}));

    public constructor(props: CameraProps) {
        const { children, scene, ...rest } = props;
        super(rest);
    ...
    public *centerOn(
        ...
        const position =
            positionOrNode instanceof Node
                ? transformVectorAsPoint(
                    positionOrNode.absolutePosition(),
    ...
    public *followCurve(
        ...
        yield* tween(duration, value => {
            ...
            const point = transformVectorAsPoint(
                curve.getPointAtPercentage(t).position,
                curve.localToWorld()
            );
    ...
    public *followCurveWithRotation(
        ...
        yield* tween(duration, value => {
            ...
            const point = transformVectorAsPoint(position, curve.localToWorld());
    ...
    protected override async drawChildren(context: CanvasRenderingContext2D) {
        await (this.scene() as any).drawChildren(context);
    }
    ...
    public override hit(position: Vector2): Node | null {
        const local = transformVectorAsPoint(position, this.localToParent());
